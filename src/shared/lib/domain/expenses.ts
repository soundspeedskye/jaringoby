...
/**
 * 예외가 정산에서 제외되려면 제안자를 제외한 그 주차의 활성 멤버 전원이
 * 승인해야 한다. 활성 멤버가 없으면 제외하지 않는다(정산 대상이 없음).
 */
export function isExceptionUnanimouslyApproved(input: {
  ...
  return input.activeMemberIds
    .filter((id) => id !== input.requesterId)
    .every((id) => input.approvedUserIds.has(id));
}

/** 예외 응답 1건. 구조적 형태만 요구해 도메인이 데이터 타입에 의존하지 않는다. */
export type ExceptionResponseRecord = {
  readonly expenseId: string;
  readonly userId: string;
  readonly decision: 'APPROVED' | 'HELD';
  readonly createdAt: string;
};

/**
 * 예외가 붙은 지출들 중 "제안자를 제외한 활성 멤버 전원 승인"으로 정산에서
 * 제외되는 지출 ID 집합.
 * 라이브 인덱스와 로컬 finalize가 같은 규칙을 쓰도록 단일 구현으로 둔다.
 *
 * - `activeMemberIdsOf`가 undefined를 반환하면 그 지출은 건너뛴다(삭제·비주차 등).
 * - `cutoffMsOf`가 주어지면 그 시각 이후 응답은 무시한다(서버 finalize의 C 마감과 동일).
...
export function collectSettlementExcludedExpenseIds(input: {
  ...
  readonly responses: readonly ExceptionResponseRecord[];
  readonly requesterIdOf: (expenseId: string) => string | undefined;
  ...
  const responsesByExpenseId = new Map<string, ExceptionResponseRecord[]>();
  for (const response of input.responses) {
    const list = responsesByExpenseId.get(response.expenseId);
    if (list) list.push(response);
    else responsesByExpenseId.set(response.expenseId, [response]);
  }

  const excluded = new Set<string>();
  for (const expenseId of input.exceptionExpenseIds) {
    const activeMemberIds = input.activeMemberIdsOf(expenseId);
    const requesterId = input.requesterIdOf(expenseId);
    if (!activeMemberIds || !requesterId) continue;
    ...
    for (const response of responsesByExpenseId.get(expenseId) ?? []) {
      if (response.decision !== 'APPROVED' || !activeMemberIdSet.has(response.userId)) continue;
      if (cutoffMs !== undefined && Date.parse(response.createdAt) > cutoffMs) continue;
      approvedUserIds.add(response.userId);
    }
    if (isExceptionUnanimouslyApproved({ activeMemberIds, requesterId, approvedUserIds })) {
