...
export function mapProfile(row: ProfileRow, signedUrls: Map<string, string>): Profile {
...
export function mapRoomPostReaction(row: RoomPostReactionRow): RoomPostReaction {
  return {
    postId: row.post_id,
    userId: row.user_id,
    emoji: row.emoji,
...
export function mapExpenseException(row: ExpenseExceptionRow): ExpenseException {
  return {
    expenseId: row.expense_id,
    reason: row.reason,
...
export function mapInvitePreview(code: string, payload: JsonObject): InvitePreview {
