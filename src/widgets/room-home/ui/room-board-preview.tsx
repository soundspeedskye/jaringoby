...
export const RoomBoardPreview = memo(function RoomBoardPreview({ roomId }: { roomId: string }) {
  ...
  const shownPosts = useMemo(
    () => [notice, latestPost, latestPoll].filter((post): post is RoomPost => Boolean(post)),
      ...
      {shownPosts.length ? (
        <View style={styles.rows}>
          {shownPosts.map((post, index) => {
            ...
            const label = post.kind === 'NOTICE' ? '공지' : post.kind === 'POLL' ? '투표글' : '최신글';
            return (
              <View key={post.id} style={[styles.row, index > 0 && styles.rowDivider]}>
                <View style={[
                  styles.label,
                ...
                <Text numberOfLines={1} style={styles.rowTitle}>{post.body}</Text>
                {post.kind === 'NOTICE' ? null : (
                  <AnimalAvatar photoUri={author?.avatarUri} size={28} value={author?.avatar} />
                )}
              </View>
            );
          })}
...
const styles = StyleSheet.create({
  ...
  rows: { overflow: 'hidden', borderWidth: 1, borderColor: palette.line, borderRadius: radii.md, backgroundColor: palette.paper },
  row: { minHeight: 50, flexDirection: 'row', alignItems: 'center', gap: spacing.sm, paddingHorizontal: spacing.md },
  rowDivider: { borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: palette.rule },
  rowTitle: { flex: 1, minWidth: 0, color: palette.ink, fontFamily: fonts.hand, fontSize: 13, fontWeight: '600' },
  label: { flexShrink: 0, paddingHorizontal: 7, paddingVertical: 3, borderRadius: radii.pill },
