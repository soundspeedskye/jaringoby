-- Zero-cost expenses are valid records (for example, a fully discounted purchase).
alter table public.expenses
  drop constraint expenses_amount,
  add constraint expenses_amount check (amount between 0 and 1000000000000);
...
begin
  ...
  if p_amount not between 0 and 1000000000000
...
create or replace function private.add_expense_impl(
  p_period_id uuid,
  p_amount bigint,
  p_category public.expense_category,
  p_occurred_at timestamptz,
...
begin
  ...
  if p_amount not between 0 and 1000000000000 then
